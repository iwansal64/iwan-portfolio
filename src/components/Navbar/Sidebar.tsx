...
export default function Sidebar() {
  ...
  <div className={`lg:hidden w-full h-full flex flex-col justify-between fixed z-100 px-4 pb-8 pt-24 duration-1000 ${isOpen ? "bg-black" : "bg-transparent pointer-events-none"}`}>
    <div className={`flex flex-col text-4xl delay-500 duration-500 ${isOpen ? "" : "delay-0! -translate-x-[100%]"}`}>
      <a href="#">1. INTRO</a>
      <a href="#about">2. ABOUT</a>
      <a href="#project">3. PROJECTS</a>
      <a href="#contact">4. CONTACT</a>
    </div>
    <div>
      <p className={`text-sm delay-500 duration-500 ${isOpen ? "" : "delay-0! -translate-x-[100%]"}`}>Developed by <span className="italic font-bold">Ridwan Bagoes Setiawan</span> with 💖</p>
    </div>
  </div>
  </>;
}